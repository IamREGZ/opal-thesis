...
import edu.cccdci.opal.databinding.FragmentOrderCancelledBinding
import edu.cccdci.opal.dataclasses.Order
import edu.cccdci.opal.firestore.FirestoreClass
import edu.cccdci.opal.layoutwrapper.WrapperLinearLayoutManager

class OrderCancelledFragment(private val isVendor: Boolean) : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentOrderCancelledBinding.inflate(inflater)
        ...
        val options = FirestoreRecyclerOptions.Builder<Order>()
            ...
            .setQuery(
                FirestoreClass().getOrderQuery(
                    this@OrderCancelledFragment, isVendor
                ),
                Order::class.java
            ).build()
        ...
        with(binding) {
            ...
            rvOrderCancelled.layoutManager = WrapperLinearLayoutManager(
                requireContext(), LinearLayoutManager.VERTICAL, false
            )

            // Create an object of Order/Sales History Adapter
            orderSalesAdapter = OrderSalesAdapter(
                this@OrderCancelledFragment, requireContext(), isVendor,
                options
            )
            // Sets the adapter of Order/Sales History (Cancelled) RecyclerView
            rvOrderCancelled.adapter = orderSalesAdapter
...
}  // end of OrderCancelledFragment class
