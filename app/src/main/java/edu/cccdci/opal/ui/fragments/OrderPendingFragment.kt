...
import edu.cccdci.opal.databinding.FragmentOrderPendingBinding
...
class OrderPendingFragment(private val isVendor: Boolean) : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentOrderPendingBinding.inflate(inflater)
        ...
        val options = FirestoreRecyclerOptions.Builder<Order>()
            ...
            .setQuery(
                FirestoreClass().getOrderQuery(
                    this@OrderPendingFragment, isVendor
                ),
                Order::class.java
            ).build()
        ...
        with(binding) {
            ...
            rvOrderPending.layoutManager = WrapperLinearLayoutManager(
                requireContext(), LinearLayoutManager.VERTICAL, false
            )

            // Create an object of Order/Sales History Adapter
            orderSalesAdapter = OrderSalesAdapter(
                this@OrderPendingFragment, requireContext(), isVendor,
                options
            )
            // Sets the adapter of Order/Sales History (Pending) RecyclerView
            rvOrderPending.adapter = orderSalesAdapter
...
}  // end of OrderPendingFragment class
