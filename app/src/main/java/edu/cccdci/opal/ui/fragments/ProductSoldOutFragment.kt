...
import edu.cccdci.opal.databinding.FragmentProductSoldOutBinding
import edu.cccdci.opal.dataclasses.Product
import edu.cccdci.opal.firestore.FirestoreClass
import edu.cccdci.opal.layoutwrapper.WrapperLinearLayoutManager

class ProductSoldOutFragment(private val marketID: String) : Fragment() {

    private lateinit var binding: FragmentProductSoldOutBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentProductSoldOutBinding.inflate(inflater)
        ...
        val options = FirestoreRecyclerOptions.Builder<Product>()
            ...
            .setQuery(
                FirestoreClass().getProductQuery(
                    this@ProductSoldOutFragment, marketID = marketID
                ),
                Product::class.java
            ).build()
        ...
        with(binding) {
            ...
            rvProductSoldOut.layoutManager = WrapperLinearLayoutManager(
                requireContext(), LinearLayoutManager.VERTICAL, false
            )

            // Create an object of Product Inventory Adapter
            productInvAdapter = ProductInventoryAdapter(
                this@ProductSoldOutFragment, requireContext(),
                requireActivity(), options
            )
            // Sets the adapter of Product Inventory (Sold Out) RecyclerView
            rvProductSoldOut.adapter = productInvAdapter
...
}  // end of ProductSoldOutFragment class
